...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  var resultadoDeLaMultiplicacion = 1
  for (var = i; i < arguments.length ; i++) {
       resultadoDeLaMultiplicacion = resultadoDeLaMultiplicacion* arguments[i];
...
function cuentoElementos(arreglo){
  ...
  //Escribe tu código aquí
var contadorDeElemetos = 0;
for (var = i ; i < arreglo.length; i++ ){
  if (arreglo[i] > 18){
    contadorDeElemetos++;
  }
} 
return contadorDeElemetos;
}
...
if (array[i] === "Enero"||array[i === "Marzo"||array[i] === "Noviembre"]){
  ...
  if (arrayConLosMesesEncontrados.length=== 3){
    ...
    return "No se han encontrado los meses pedidos";
  }
}
...
function mayorACien(array) {
  ...
  // Tu código:
 var arrayConNumerosMayoresACien= []
  for (var i =0; i < array.length; i++){
    if (array[i] > 100){
      arrayConNumerosMayoresACien.push(array[i]);
    }
  }return arrayConNumerosMayoresACien;
}
